...
type MarkdownReportGenerator interface {
	GetMarkdownReport() *MarkdownReport
	GenerateReport() []byte
}
...
func NewMarkdownReport(
	disableTimestamp bool,
	generated time.Time,
	history []*Commit) MarkdownReportGenerator {

	var reportItems []*MarkdownReportItem

	for x := range history {
		historyItem := history[x]

		if historyItem.Changes != nil {
			_, stats := BuildTree(historyItem.Changes)
			...
			var markdownChanges []*MarkdownChange
	...
	var totalStats ChangeStatistics
...
func (md *markdownReport) GetMarkdownReport() *MarkdownReport {
	return md.model
}
...
func (md *markdownReport) GenerateReport() []byte {
	...
	for i, item := range md.model.ReportItems {
		...
		if len(item.Changes) > 0 {
			...
			changesByTypeAndProperty := make(map[string]map[string][]*MarkdownChange)
			for _, change := range item.Changes {
				if changesByTypeAndProperty[change.Type] == nil {
					changesByTypeAndProperty[change.Type] = make(map[string][]*MarkdownChange)
				}
			...
			for _, changeType := range changeTypeOrder {
				if changesByProperty, exists := changesByTypeAndProperty[changeType]; exists {
					...
					for _, property := range properties {
						...
						changesByDescription := make(map[string][]*MarkdownChange)
...
func generateSummary(stats *ChangeStatistics) string {
	if stats.Total == 0 {
