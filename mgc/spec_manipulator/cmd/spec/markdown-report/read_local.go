...
func ExtractHistoryFromFile(repoDirectory, filePath, baseCommit string,
	progressChan chan *ProgressUpdate, errorChan chan ProgressError, globalRevisions bool, limit int, limitTime int,
) ([]*Commit, []error) {
	...
	if err != nil {
		...
		SendProgressError("git", errString, errorChan)
	...
	SendProgressUpdate("extraction",
		fmt.Sprintf("%d commits extracted", len(commitHistory)), true, progressChan)
	return commitHistory, nil
}

func PopulateHistoryWithChanges(commitHistory []*Commit, limit int, limitTime int,
	progressChan chan *ProgressUpdate, errorChan chan ProgressError, base string, remote, extRefs bool,
) ([]*Commit, []error) {
...
func BuildCommitChangelog(commitHistory []*Commit,
	progressChan chan *ProgressUpdate, errorChan chan ProgressError, base string, remote, extRefs bool,
) ([]*Commit, []error) {
	...
	for c := len(commitHistory) - 1; c > -1; c-- {
		...
		if len(oldBits) > 0 && len(newBits) > 0 {
			...
			if err != nil {
				SendFatalError("building models", fmt.Sprintf("unable to parse original document '%s': %s", commitHistory[c].FilePath, err.Error()), errorChan)
				changeErrors = append(changeErrors, err)
				return nil, changeErrors
			} else {
				SendProgressUpdate("building models",
			...
			if err != nil {
				SendProgressError("building models", fmt.Sprintf("unable to parse modified document '%s': %s", commitHistory[c].FilePath, err.Error()), errorChan)
				changeErrors = append(changeErrors, err)
			}
			...
			if oldDoc != nil && newDoc != nil {
				...
				if errs != nil {
					SendProgressError("building models", fmt.Sprintf("Error thrown when comparing: %s", errs[0].Error()), errorChan)
					changeErrors = append(changeErrors, errs...)
				}
				commitHistory[c].Changes = changes
			} else {
				SendProgressError("building models", "No OpenAPI documents can be compared!", errorChan)
				changeErrors = append(changeErrors, err)
			}
		}
		if len(oldBits) == 0 && len(newBits) > 0 {
			newDoc, err = libopenapi.NewDocumentWithConfiguration(newBits, docConfig)
			if err != nil {
				SendFatalError("building models", fmt.Sprintf("unable to create OpenAPI modified document: %s", err.Error()), errorChan)
				changeErrors = append(changeErrors, err)
				return nil, changeErrors
			}
		}
